...
app.post('/webhook', async (req, res) => {
  try {
    ...
    if (parsed.cmd === 'income' || parsed.cmd === 'expense') {
      ...
      const type = parsed.cmd;
      await pool.query(`INSERT INTO transactions (phone,type,amount,category,note) VALUES ($1,$2,$3,$4,$5)`,
        [phone, type, amount, type, parsed.text]);
      reply += randomVegeta(type) + ` (${type==='income'?'+':'-'}R$${amount.toFixed(2)})`;
      ...
      else { const loan=loanRes.rows[0]; await pool.query(`UPDATE loans SET paid=true WHERE id=$1`, [loan.id]); reply+=`✅ Empréstimo de R$${parseFloat(loan.amount).toFixed(2)} marcado como pago.`; }
    } else if (parsed.cmd === 'addbill') {
      const parts = parsed.text.split(/\s+/); const day=parts[2]||null; const desc=parts[3]||'fatura'; const amount=extractAmount(parsed.text)||0;
      let due_date=null; if(day && /^\d{1,2}$/.test(day)){ const d=new Date(); d.setDate(parseInt(day)); due_date=d.toISOString().slice(0,10); }
      await pool.query(`INSERT INTO bills (phone,description,amount,due_date) VALUES ($1,$2,$3,$4)`,[phone,desc,amount,due_date]);
...
cron.schedule('0 8 * * *', async () => {
  try {
    ...
    const resSum = await pool.query(`SELECT phone, COALESCE(SUM(CASE WHEN type='income' THEN amount ELSE -amount END),0) AS balance FROM transactions GROUP BY phone`);
    for(const row of resSum.rows){
      await sendWhatsApp(row.phone, `${vegetaReplyBase()} Resumo diário: Saldo R$${parseFloat(row.balance||0).toFixed(2)}`);
    }
  } catch(e){ console.error('Daily summary job error', e); }
...
cron.schedule('0 * * * *', async ()=>{
  try{
    ...
    for(const loan of resLoans.rows){
      ...
      if(diffDays <= 1){
        await sendWhatsApp(loan.phone, `${vegetaReplyBase()} ${randomVegeta('reminder')} Empréstimo R$${parseFloat(loan.amount).toFixed(2)} vence em ${diffDays} dia(s).`);
      }
    }
  }catch(e){ console.error('Loan reminder job error', e);}
},{timezone:'America/Sao_Paulo'});
  ...
  try{
    ...
    for(const bill of resBills.rows){
      ...
      if(diffDays>=0 && diffDays<=2){
        await sendWhatsApp(bill.phone, `${vegetaReplyBase()} Lembrete: ${bill.description} R$${parseFloat(bill.amount).toFixed(2)} vence em ${diffDays} dia(s).`);
      }
    }
  }catch(e){ console.error('Bills reminder job error', e);}
},{timezone:'America/Sao_Paulo'});

// ===== Start server =====
initDb().then(()=>{
  app.listen(PORT, ()=>console.log(`Peartree Vegeta running on port ${PORT}`));
